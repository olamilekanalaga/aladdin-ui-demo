...
import {DevnetBuyModal} from "@/app/components/devnet-market/DevnetBuyModal";
...
export function LaunchesExperience(){
  ...
  const [tab,setTab]=useState<Tab>("New");const [filters,setFilters]=useState<LaunchFilterState>(defaultLaunchFilters);const [sort,setSort]=useState<LaunchSortKey>("newest");const [filtersOpen,setFiltersOpen]=useState(false);const [selectedBuy,setSelectedBuy]=useState<LaunchToken|null>(null);const [hydrated,setHydrated]=useState(false);
  ...
  const onBuy=(token:LaunchToken)=>{setSelectedBuy(token);if(!wallet.connected)wallet.openWalletModal({reason:"$"+token.symbol+" Buy · resume this token after connection"})};
  return <><div className="pageTitle"><div><span className="eyebrow">MARKET DISCOVERY</span><h1>Launches</h1><p>Find activity, then investigate the behaviour underneath it.</p></div><div className="titleStats"><span><b>{launchTokens.length}</b> tracked</span><span><b>{launchTokens.filter(x=>x.lifecycle==="almost-bonded").length}</b> almost bonded</span><span><b>{launchTokens.filter(x=>x.lifecycle==="migrated").length}</b> migrated</span></div></div>
    ...
    <DevnetBuyModal token={selectedBuy} onClose={()=>setSelectedBuy(null)}/><LaunchFilters open={filtersOpen} filters={filters} onApply={setFilters} onClose={()=>setFiltersOpen(false)}/></>;
}
