...
export function LaunchFeedRow({token}:{token:LaunchToken}){
  const change=volumeChange(token);const migrated=token.lifecycle==="migrated";const fresh=token.ageMinutes<=15;
  return <article className={`launchFeedRow lifecycle-${token.lifecycle}`}><Link className="launchFeedImage" href={`/live/${token.id}`} aria-label={`Open ${token.symbol} in Terminal`}><SyntheticTokenImage id={token.id} symbol={token.symbol} color={token.color} lifecycle={token.lifecycle} variant="feed"/></Link><div className="launchFeedBody">
    <header className="launchFeedIdentity"><Link href={`/live/${token.id}`}>${token.symbol}</Link><span className={fresh?"isFresh":""}>{migrated?`Migrated ${ageLabel(token.migratedMinutesAgo??0)}`:ageLabel(token.ageMinutes)}</span></header><div className="launchFeedName">{token.name}</div>
