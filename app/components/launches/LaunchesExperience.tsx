...
import type {LaunchFilters as LaunchFilterState,LaunchSortKey,LaunchToken} from "@/app/types/launches";
...
import {LaunchFeedRow} from "@/app/components/launches/LaunchFeedRow";import {useAladdinWallet} from "@/app/hooks/useAladdinWallet";
...
export function LaunchesExperience(){
  ...
  const [tab,setTab]=useState<Tab>("New");const [filters,setFilters]=useState<LaunchFilterState>(defaultLaunchFilters);const [sort,setSort]=useState<LaunchSortKey>("newest");const [filtersOpen,setFiltersOpen]=useState(false);const [buyNotice,setBuyNotice]=useState<string|null>(null);const [hydrated,setHydrated]=useState(false);
  ...
  const onBuy=(token:LaunchToken)=>{if(!wallet.connected){wallet.openWalletModal({reason:"$"+token.symbol+" Buy · resume this token after connection"});return}setBuyNotice("$"+token.symbol+" is ready for a future Devnet transaction. Execution is not enabled yet.");window.setTimeout(()=>setBuyNotice(null),3200)};
  return <><div className="pageTitle"><div><span className="eyebrow">MARKET DISCOVERY</span><h1>Launches</h1><p>Find activity, then investigate the behaviour underneath it.</p></div><div className="titleStats"><span><b>{launchTokens.length}</b> tracked</span><span><b>{launchTokens.filter(x=>x.lifecycle==="almost-bonded").length}</b> almost bonded</span><span><b>{launchTokens.filter(x=>x.lifecycle==="migrated").length}</b> migrated</span></div></div>
    ...
    {visible.length?<div className="launchCardGrid">{visible.map(token=><div className="launchResponsiveItem" key={token.id}><LaunchTokenCard token={token} onBuy={onBuy}/><LaunchFeedRow token={token} onBuy={onBuy}/></div>)}</div>:<div className="launchEmpty"><b>No launches match these filters.</b><span>Try widening the market or behaviour conditions.</span><button onClick={()=>setFilters(defaultLaunchFilters)}>Clear filters</button></div>}<div className="footnote">Simulated launch records · not live market activity</div>
    {buyNotice&&<div className="launchToast" role="status">{buyNotice}</div>}<LaunchFilters open={filtersOpen} filters={filters} onApply={setFilters} onClose={()=>setFiltersOpen(false)}/></>;
}
