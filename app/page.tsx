...
export default function Home(){
  ...
  return <div className="publicLanding">
    ...
    <main className="publicHero" id="product"><div className="heroCopy"><span className="eyebrow">SOLANA TRADING INTELLIGENCE</span><h1>Trade the behaviour<br/>underneath the market.</h1><p>Aladdin resolves tokens, wallets and transactions into decision-ready evidence—then gives traders, analysts and token creators a structured Terminal and adaptive Ask Aladdin reasoning layer.</p><div className="connectLabel">CONNECT OR CONTINUE WITH</div><div className="connectOptions"><button className="phantom" onClick={()=>connect("Phantom")}><Wallet/>Phantom</button><button onClick={()=>connect("Google")}><b>G</b>Google</button><button onClick={()=>connect("Telegram")}><MessageCircle/>Telegram</button></div>{notice&&<p className="landingNotice">{notice}</p>}<small className="mockNote">Synthetic demo data — not live or historical Solana activity. Authentication and wallet connection are not live yet.</small></div><div className="heroVisual"><Image src="/aladdin-logo.jpg" width={430} height={430} alt="Aladdin Intelligence" priority/><div><span>SEARCH</span><i/ ><span>TERMINAL</span><i/><span>ASK ALADDIN</span></div></div></main>
